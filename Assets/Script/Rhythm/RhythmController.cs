...
namespace SonicBloom.Koreo.Demos
{
	...
	public class RhythmController : MonoBehaviour
	{
		...
		public NoteObjectCustom noteObjectArchetype;
		...
		Stack<NoteObjectCustom> noteObjectPool = new Stack<NoteObjectCustom>();
		...
		Queue<NoteObjectCustom> trackedNotes = new Queue<NoteObjectCustom>();
		...
		public NoteObjectCustom GetFreshNoteObject()
		{
			NoteObjectCustom retObj;
			...
			//else
			//{
				retObj = GameObject.Instantiate<NoteObjectCustom>(noteObjectArchetype);
			//}
		...
		public void ReturnNoteObjectToPool(NoteObjectCustom obj)
		{
			if (obj != null)
			{
				obj.enabled = false;
		...
		void CheckSpawnNext()
		{
			...
			Debug.Log("laneEvents[pendingEventIdx].StartSample " + laneEvents[pendingEventIdx].StartSample);
			// Spawn for all events within range.
			while (pendingEventIdx < laneEvents.Count &&
				   laneEvents[pendingEventIdx].StartSample < currentTime + ShowTimeOffSet)
			{
				KoreographyEvent evt = laneEvents[pendingEventIdx];

				NoteObjectCustom newObj = GetFreshNoteObject();
				newObj.Initialize(evt, Color.blue,this);
